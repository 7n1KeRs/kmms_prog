...
using dnv::LongNumber;

LongNumber::LongNumber() { // конструктор по уполчанию
    numbers = new int[1]{0};
    length = 1;
    sign = 1;
}

LongNumber::LongNumber(int length, int sign) { // конструктор с параметрами
...
LongNumber::LongNumber(const char* const str) { // конструктор с параметрами
    if (!str) {
        numbers = new int[1]{0};
        length = 1;
        sign = 1;
...
LongNumber::LongNumber(int inter){
	if (!inter){
		numbers = new int[1]{0};
		length = 1; 
		sign = 1;
...
LongNumber::LongNumber(const LongNumber& x) { // конструкор копирования 
    sign = x.sign;
    length = x.length;
    numbers = nullptr;
...
LongNumber::LongNumber(LongNumber&& x) { // конструкор перемещения
    sign = x.sign;
    length = x.length;
    numbers = x.numbers;
...
LongNumber LongNumber::operator + (const LongNumber& x) const {
    ...
    if (sign == x.sign)
    {
        ...
        if (res.length > 1 && res.numbers[res.length - 1] == 0)
            res.length--;
    }
    else
    {
        ...
        for (int j = 0; j < length; j++) {
            ...
            if (res.numbers[i + j] > 9)
            {
                ...
                if (i + j + 1 < res.length) {
                    ...
                    int* new_numbers = new int[res.length + 1];
                    for (int k = 0; k < res.length; k++)
                        new_numbers[k] = res.numbers[k];
                    new_numbers[res.length] = r;
                    res.numbers[i + j] -= r * 10;
                    delete[] res.numbers;
                    res.numbers = new_numbers;
                    res.length++;
                }
            }
        }
...
LongNumber LongNumber::operator / (const LongNumber& x) const {
    ...
    if (!numbers || !x.numbers || x.length == 1 && x.numbers[0] == 0)
        return LongNumber();
    ...
    else 
    {
        ...
        for(int i = 0; i < res.length; i++) 
        {
            ...
            for (int j = 0; j < x.length; j++) {
                if (length - x.length - i + j >= 0 && length - x.length - i + j < divisor.length)
...
LongNumber LongNumber::operator % (const LongNumber& x) const {
    // проверка на nullptr и деление на ноль
    if (!numbers || !x.numbers || x.length == 1 && x.numbers[0] == 0)
        return LongNumber();
