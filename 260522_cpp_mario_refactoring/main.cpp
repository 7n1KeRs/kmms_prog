...
void create_level(int current_level, GameObject& player, GameObject*& background_elem, int& background_elems_count, GameObject*& enemy, int& enemies_count, int& score, bool reset_score = true);
...
GameObject* add_new_background_elem(GameObject*& background_elem, int& background_elems_count) //
{	
	GameObject* temp = (GameObject*)realloc(background_elem, sizeof(GameObject) * (background_elems_count + 1));
    if (temp != nullptr)
    {
...
GameObject* add_new_enemy(GameObject*& enemy, int& enemies_count)
{
	GameObject* temp = (GameObject*)realloc(enemy, sizeof(GameObject) * (enemies_count + 1));
    if (temp != nullptr)
    {
        enemy = temp;
        enemies_count++;
...
void create_level(int current_level, GameObject& player, GameObject*& background_elem, int& background_elems_count, GameObject*& enemy, int& enemies_count, int& score, bool reset_score)
{
	system("color 9F");
...
void kill_player(GameObject& player, GameObject*& background_elem, int& background_elems_count, GameObject*& enemy, int& enemies_count, int current_level, int& score)
{
	system("color 4F");
	Sleep(500);
	create_level(current_level, player, background_elem, background_elems_count, enemy, enemies_count, score, true);
}
...
void remove_enemy(int index, GameObject*& enemy, int& enemies_count)
{	
	enemy[index] = enemy[enemies_count - 1];
	...
	for (int i = 0; i < background_elems_count; i++)
		if (check_collision(*obj, background_elem[i]))
		{
			...
			if ((background_elem[i].kind == '?') && (obj->vertical_speed < 0) && (obj == &player)) //
			{
                ...
                GameObject* new_enemy = add_new_enemy(enemy, enemies_count);
                if (new_enemy != nullptr)
                {
                    init_object(new_enemy, background_elem[i].x, background_elem[i].y - 3, 3, 2, '$');
                    new_enemy->vertical_speed = -0.7;
				}
			}
			...
			if (background_elem[i].kind == '+')
			{
				...
				system("color 2F");
				Sleep(500);
				create_level(current_level, player, background_elem, background_elems_count, enemy, enemies_count, score, false);
			}
			break;
		}
}